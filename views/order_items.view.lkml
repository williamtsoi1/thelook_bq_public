  ...
  dimension: order_id {
    ...
    action: {
      ...
      form_param: {
        ...
        type: textarea
        default: "Hey,
        Could you check out order #{{value}}. It's saying its {{orders.status._value}},
...
########## Logistics ##########
  ...
  dimension: days_to_process {
    ...
    type: number
    sql: CASE
        WHEN ${orders.status} = 'Processing' THEN TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), ${created_raw}, DAY)*1.0 + 1
        WHEN ${orders.status} IN ('Shipped', 'Complete', 'Returned') THEN TIMESTAMP_DIFF(${shipped_raw}, ${created_raw}, DAY)*1.0
        WHEN ${orders.status} = 'Cancelled' THEN NULL
      END
       ;;
  }
