- dashboard: shipping_logistics__operations_overview
  ...
  - title: Order Shipment Status
    ...
    explore: order_items
    type: looker_column
    fields: [order_items.created_date, orders.status, order_items.order_count]
    pivots: [orders.status]
    filters:
      order_items.created_date: 60 days
      order_items.status: Complete,Shipped,Processing
    sorts: [order_items.created_date desc, orders.status]
    limit: 500
    column_limit: 50
  ...
  - title: Open Orders >3 Days Old
    ...
    explore: order_items
    type: looker_grid
    fields: [order_items.order_id, users.email, order_items.created_date, orders.status,
      products.name, order_items.days_to_process]
    filters:
